...
def classify_311_request(request_text: str) -> dict:
    ...
    open_ai_client = _get_openai_client()
    ...
    model=os.environ.get("AZURE_OPENAI_DEPLOYMENT","gpt-4o"),
    messages=[{"role":"system","content":"Classify the following 311 service request into one of these categories: Pothole, Noise Complaint, Trash/Litter, Street Light, Water/Sewer, Other. Return a JSON object with keys: category, confidence (a number in the range 0.0 to 1.0 (not a percentage)), reasoning (a short explanation (string))."},
...
def check_content_safety(text: str) -> dict:
    ...
    content_safety_client = _get_content_safety_client()
    ...
    result = {
        "safe": all(item['severity'] == 0 for item in response['categoriesAnalysis']),
        "categories": {category.category: category.severity for category in response.categories_analysis},
    }
    return result
